<div id="delete-personnel-{{ $person->id }}" tabindex="-1"
    class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                    Hapus Tenaga Pengasuh dan Fasilitator
                </h3>
                <button type="button"
                    class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
                    data-modal-hide="delete-personnel-{{ $person->id }}">
            ...
            <form action="{{ route('personnel.destroy', $person->id) }}" method="POST" id="delete-personnel-form">
                @csrf
                @method('DELETE')
                <div class="p-6 space-y-6">
                    <p class="text-sm text-gray-700 dark:text-gray-300">Anda yakin ingin menghapus Tenaga Pengasuh dan
                        Fasilitator ini?
                    </p>
                </div>
                <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                    ...
                    <button type="button" data-modal-hide="delete-personnel-{{ $person->id }}"
